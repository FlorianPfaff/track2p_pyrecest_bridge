...
def test_activity_similarity_favors_matching_trace_structure() -> None:
    ...
    assert components["activity_similarity"][0, 0] > components["activity_similarity"][0, 1]
    assert components["activity_similarity"][1, 1] > components["activity_similarity"][1, 0]
...
def test_activity_similarity_prefers_spike_traces_in_auto_mode() -> None:
    ...
    reference = CalciumPlaneData(roi_masks=masks, traces=fluorescence, spike_traces=spikes)
    measurement = CalciumPlaneData(roi_masks=masks, traces=fluorescence, spike_traces=spikes)
...
def test_activity_similarity_components_can_be_added_to_pairwise_component_dict() -> None:
    masks = _masks()
    ...
    npt.assert_allclose(features[:, :, 0], pairwise_components["activity_similarity_cost"])
    npt.assert_allclose(features[:, :, 1], pairwise_components["activity_similarity_available"])
